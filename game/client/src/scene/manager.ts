...
import { Avatar, resolveCharacterResources, loadCharacterTextures, type TextureCache } from "./avatar.js";
...
export class SceneManager {
  ...
  private spawnAvatar(snapshot: AvatarSnapshot, isLocal: boolean): Avatar | null {
    ...
    // Resolve character resources by tag lookup
    const charResources = resolveCharacterResources(snapshot.characterId, this.gameData);
    if (charResources.all.length === 0) {
      console.warn(`[Scene] No resources found for character "${snapshot.characterId}" (avatar ${snapshot.id})`);
      return null;
    }

    // Load character textures
    loadCharacterTextures(charResources, this.gameData, this.textureCache, getCachedImage);

    // Create avatar
    const avatar = new Avatar(snapshot, charResources, this.gameData, this.textureCache, isLocal);
