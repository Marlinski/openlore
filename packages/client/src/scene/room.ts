...
export class RoomScene {
  ...
  private loadRoomTextures(room: RoomDefinition, gameData: ProjectData): void {
      ...
      if (p.compositeId) {
        ...
        if (comp) {
          for (const part of comp.parts) {
            neededIds.add(part.region.tilesetId);
          }
        }
      }
    }
  ...
  private buildSprites(room: RoomDefinition, gameData: ProjectData): void {
  ...
  private addFloorPlacement(p: TexturePlacement, gameData: ProjectData): void {
    if (p.compositeId) {
      const comp = gameData.composites.find((c) => c.id === p.compositeId);
      if (!comp) return;
  ...
  private addObjectPlacement(p: TexturePlacement, gameData: ProjectData): void {
    if (p.compositeId) {
      const comp = gameData.composites.find((c) => c.id === p.compositeId);
      if (!comp) return;
      for (const part of comp.parts) {
